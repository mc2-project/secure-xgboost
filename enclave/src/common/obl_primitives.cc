...
// TODO(andrew): define this as inline cause segment fault. Need to know why. Is it due
...
struct Generic {
    ...
    Generic(double x, int16_t y, double z)
        : x(x), y(y), z(z) {}
...
void test_ObliviousAssign() {
    ...
    test(" (false, (uint8_t) 1, (uint8_t) 2) ",
        ObliviousChoose(false, (uint8_t) 1, (uint8_t) 2) == 2);
...
void test_ObliviousSort() {
    ...
    Generic g_arr[5] = {Generic(-1.35, 2, 3.21),
      Generic(4.123, 5, 6.432),
      Generic(-5.123, 3, 7.432),
      Generic(6.123, 1, 1.432),
      Generic(-3.123, 4, 0.432)};
