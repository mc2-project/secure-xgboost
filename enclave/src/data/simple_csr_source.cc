...
void SimpleCSRSource::CopyFrom(std::vector<std::unique_ptr<dmlc::Parser<uint32_t>>> parsers) {
  for (auto parser = std::begin(parsers).get(); parser != std::end(parsers).get(); ++parsers) {
