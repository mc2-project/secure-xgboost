...
class EnclaveContext {
  ...
  */
  ...
  bool verifySignature(uint8_t* data, size_t data_len, uint8_t* signature, size_t sig_len) {
    ...
    if ((ret = mbedtls_pk_parse_public_key(
            &_pk_context, (const unsigned char*) key, strlen(key) + 1)) != 0) {
    ...
    if ((ret = mbedtls_pk_verify(
            &_pk_context, MBEDTLS_MD_SHA256, hash, 0, signature, sig_len)) != 0) {
  ...
  bool decrypt_and_save_client_key(
      uint8_t* data, size_t data_len, uint8_t* signature, size_t sig_len) {
    if (rabit::GetRank() == 0) {
 ...
 private:
  ...
  bool generate_public_key() {
    ...
    // FIXME: Write out the private key in PEM format for exchange with other enclaves.
    /*
 ...
 public:
  ...
  static int compute_sha256(const uint8_t* data, size_t data_len, uint8_t sha256[32]) {
    int ret = 0;
