 ...
 *
 * \file c-api-demo.c
 * \brief A simple example of using mc2-xgboost C API.
 */
...
int main(int argc, char** argv) {
  ...
  safe_xgboost(get_remote_report_with_pubkey(
  ...
  safe_xgboost(verify_remote_report_and_set_pubkey(
        pem_key, key_size, remote_report, remote_report_size));

  uint8_t* encrypted_data = reinterpret_cast<uint8_t*>(malloc(1024*sizeof(uint8_t)));
  size_t encrypted_data_size = 1024;
  uint8_t* signature = reinterpret_cast<uint8_t*>(malloc(1024*sizeof(uint8_t)));
  size_t sig_len;

  safe_xgboost(encrypt_data_with_pk(
        test_key, CIPHER_KEY_SIZE, pem_key, key_size, encrypted_data, &encrypted_data_size));
  safe_xgboost(sign_data(
        "keypair.pem", encrypted_data, encrypted_data_size, signature, &sig_len));
  /*
  verifySignature("publickey.crt", encrypted_data, encrypted_data_size, signature, sig_len);

  safe_xgboost(add_client_key((char*)fname1.c_str(),
        encrypted_data, encrypted_data_size, signature, sig_len));
  safe_xgboost(add_client_key((char*)fname2.c_str(),
        encrypted_data, encrypted_data_size, signature, sig_len));
  */

  std::cout << "Adding key\n";
  safe_xgboost(add_client_key(
        encrypted_data, encrypted_data_size, signature, sig_len));
