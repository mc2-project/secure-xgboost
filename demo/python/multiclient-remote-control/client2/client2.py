...
def run(channel_addr, sym_key_file, priv_key_file, cert_file):
    ...
    dtrain = xgb.DMatrix({"user1": HOME_DIR + "demo/python/multiclient-rpc/data/c1_train.enc", username: HOME_DIR + "demo/python/multiclient-rpc/data/c2_train.enc"}, encrypted=True)
    if not dtrain:
        print("Error loading data")
        return

    print("Creating test matrix")
    dtest1 = xgb.DMatrix({"user1": HOME_DIR + "demo/python/multiclient-rpc/data/c1_test.enc"}, encrypted=True)
    dtest2 = xgb.DMatrix({username: HOME_DIR + "demo/python/multiclient-rpc/data/c2_test.enc"}, encrypted=True)
